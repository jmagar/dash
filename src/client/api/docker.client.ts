...
export async function getContainers(): Promise<ApiResult<Container[]>> {
  try {
    ...
    logger.info('Containers fetched successfully', { count: response.data?.data?.length });
    return response.data;
  } catch (error) {
    return handleApiError<Container[]>(error, 'getContainers');
  }
}
...
export async function createStack(name: string, composeFile: string): Promise<ApiResult<void>> {
  try {
    ...
    const response = await api.post(API_ENDPOINTS.DOCKER.STACKS, {
      name,
      composeFile,
    });
...
export async function getStackComposeFile(name: string): Promise<ApiResult<string>> {
  try {
    ...
    const response = await api.get(`${API_ENDPOINTS.DOCKER.STACK(name)}/compose`);
...
export async function updateStackComposeFile(name: string, composeFile: string): Promise<ApiResult<void>> {
  try {
    ...
    const response = await api.put(`${API_ENDPOINTS.DOCKER.STACK(name)}/compose`, {
      composeFile,
    });
