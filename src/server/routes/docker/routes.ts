...
const getContainers = async (req, res) => {
  ...
  try {
    ...
    logRouteAccess('Failed to get Docker containers:', metadata);
...
const getStacks = async (req, res) => {
  ...
  try {
    ...
    logRouteAccess('Failed to get Docker stacks:', metadata);
 ...
 */
const cacheContainers = async (req, res) => {
  const hostId = parseInt(req.params.hostId, 10);
  if (isNaN(hostId)) {
    const error = createApiError('Invalid host ID', null, 400);
    return res.status(400).json({
  ...
  try {
    ...
    logRouteAccess('Failed to cache Docker containers:', metadata);
 ...
 */
const cacheStacks = async (req, res) => {
  const hostId = parseInt(req.params.hostId, 10);
  if (isNaN(hostId)) {
    const error = createApiError('Invalid host ID', null, 400);
    return res.status(400).json({
  ...
  try {
    ...
    logRouteAccess('Failed to cache Docker stacks:', metadata);
