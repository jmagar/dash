...
export type MetricName = 'cpu' | 'memory' | 'latency' | 'errorRate' | 'requestCount' | 'activeConnections' | string;
...
export class ServiceMetricsCollector extends BaseService {
  ...
  private readonly thresholds: Map<MetricName, number>;
   ...
   */
  public recordMetric(serviceName: string, name: MetricName, value: number, labels?: Record<string, string>): void {
   ...
   * Record an error
   */
  public recordError(serviceName: string, error: Error): void {
   ...
   */
  public setThreshold(metricName: MetricName, value: number): void {
  ...
  protected async onStart(): Promise<void> {
    const config = this.config as MetricsConfig;
