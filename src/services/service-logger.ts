...
export class ServiceLogger extends BaseService {
  ...
  constructor(
    config: ServiceConfig & { maxLogs?: number; flushInterval?: number }
  ) {
    super({
      name: 'service-logger',
      version: '1.0.0',
  ...
  protected async onStart(): Promise<void> {
    // Set up periodic log flushing if configured
    const flushInterval = (this.config as any).flushInterval;
    if (flushInterval) {
